...
class TestMl2AgentsNetworkAccess(Ml2PluginV2TestCase):
    ...
    def _test_get_agent_to_mech_driver(self, agent_type, expected_driver):
        for agent in self.driver.get_agents(self.context):
            if agent['agent_type'] == agent_type:
                break
        else:
            self.fail('No %s agent' % agent_type)

        driver = self.driver.mechanism_manager.get_agent_to_mech_driver(agent)
        self.assertEqual(expected_driver, driver)

    def test_get_agent_to_mech_driver(self):
        self._test_get_agent_to_mech_driver(
            constants.AGENT_TYPE_OVS,
            self.driver.mechanism_manager.mech_drivers['openvswitch'].obj)

    def test_get_agent_to_no_mech_driver(self):
        helpers.register_dhcp_agent()
        self._test_get_agent_to_mech_driver(
